...
def basic_pagination_examples():
    ...
    for debt in page1.data[:3]:  # Show first 3
        print(f"  Debt {debt.id}: {debt.status} - ${debt.total_amount/100:.2f}")
    ...
    # With expanded data
    print("\n--- With Expanded Data ---")
    expanded_page = client.debts.list(
        limit=3,
        expand=["customer", "organisation"]
    )
...
def generator_iteration_examples():
    ...
    for debt in client.debts.iterate(limit_per_page=10, max_pages=2):
        count += 1
        print(f"  {count:2d}. Debt {debt.id}: {debt.status}")
    ...
    for debt in client.debts.iterate(limit_per_page=20, max_pages=2):
        stats["total"] += 1
        stats["total_amount"] += debt.total_amount
...
def search_pagination_examples():
    ...
    print("\n--- Basic Search Pagination ---")
    search_results = client.debts.search(
        query="status:paying",
        limit=5
    )
    ...
    try:
        ...
        for debt in client.debts.iterate_search(
            ...
            count += 1
            print(f"  High-value debt {debt.id}: ${debt.total_amount/100:.2f}")
            if count >= 3:
...
def chunked_processing_example():
    ...
    for debt in client.debts.iterate(limit_per_page=10, max_pages=2):
        ...
        if len(chunk) >= chunk_size:
            ...
            total_amount = sum(debt.total_amount for debt in chunk)
            statuses = [str(debt.status) for debt in chunk]
...
def advanced_patterns():
    ...
    for debt in client.debts.iterate(limit_per_page=15, max_pages=2):
        ...
        if debt.total_amount > 10000:  # $100+
            processed += 1
            print(f"  Processing high-value debt {debt.id}: ${debt.total_amount/100:.2f}")
        else:
            skipped += 1
...
def performance_comparison():
    ...
    print("\n" + "=" * 60)
    print("7. PERFORMANCE & MEMORY COMPARISON")
    print("=" * 60)
    ...
    1. Load All at Once:
       debts = client.debts.list(limit=1000).data
       Memory: HIGH (all 1000 objects in memory)
       Use case: Small datasets, need random access
    
    2. Manual Pagination:
       for page in manual_pagination():
           process(page.data)
       Memory: MEDIUM (one page at a time)
       Use case: When you need page-level control
    
    3. Generator Iteration:
       for debt in client.debts.iterate():
           process(debt)
       Memory: LOW (constant, regardless of dataset size)
       Use case: Large datasets, sequential processing
    
    ✅ Recommended: Use iterate() for most use cases
    """)
    ...
    # This will only keep ~20 objects in memory at any time
    for debt in client.debts.iterate(limit_per_page=20, max_pages=3):
        total_processed += 1
        # Process one debt at a time
        # Memory usage stays constant
        
        if total_processed % 15 == 0:
            print(f"  Processed {total_processed} debts (memory usage: ~20 objects)")
    
    print(f"  Final total: {total_processed} debts processed")
